...
import com.iotcitybackend.application.service.SensorDataService;
import com.iotcitybackend.application.service.DeviceService;
...
public class SensorDataController {
    ...
    @Operation(
        ...
        description = """
            Recebe e armazena dados de um sensor IoT.
            
            **Campos obrigatórios:**
            - `deviceId`: ID do dispositivo
            - `sensorType`: Tipo do sensor (TEMPERATURE, HUMIDITY, etc.)
            - `value`: Valor medido pelo sensor
            
            **Campos opcionais:**
            - `unit`: Unidade de medida (CELSIUS, PERCENTAGE, etc.)
            - `latitude`: Latitude da localização
            - `longitude`: Longitude da localização
            - `timestamp`: Data/hora da medição (preenchido automaticamente se não fornecido)
            
            **Campos preenchidos automaticamente:**
            - `id`: ID único do registro
            - `createdAt`: Data/hora do registro no sistema
            """
    )
    @ApiResponses(value = {
        @ApiResponse(
    ...
    public ResponseEntity<?> receiveSensorData(
            @Parameter(description = "ID do dispositivo", example = "1") @RequestParam Long deviceId,
            @Parameter(description = "Tipo do sensor", example = "TEMPERATURE") @RequestParam String sensorType,
            @Parameter(description = "Valor medido pelo sensor", example = "25.5") @RequestParam Double value,
            @Parameter(description = "Unidade de medida", example = "CELSIUS") @RequestParam(required = false) String unit,
            @Parameter(description = "Latitude da localização", example = "-5.7793") @RequestParam(required = false) Double latitude,
            @Parameter(description = "Longitude da localização", example = "-35.2009") @RequestParam(required = false) Double longitude,
            @Parameter(description = "Data/hora da medição", example = "2024-03-19T10:30:00") @RequestParam(required = false) LocalDateTime timestamp) {
